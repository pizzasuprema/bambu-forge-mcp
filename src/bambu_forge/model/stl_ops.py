    ...
    for i, op in enumerate(operations):
        ...
        try:
            if op_type == "scale":
                ...
                return {"status": "error", "error_code": "INVALID_OPERATION", "message": f"Unknown operation: {op_type}"}
        except (KeyError, ValueError) as e:
            return {"status": "error", "error_code": "INVALID_OPERATION", "message": f"Operation {i} ({op_type}): missing key or invalid value — {e}"}
